...
object Practice
{
  /*def listLen[A](input: List[A]): Int = {
	...
	def runLENC(input: List[Char]): List[(Char,Int)] = {
		...
		accum(input,1)
	}                                         //> runLENC: (input: List[Char])List[(Char, Int)]
	def runLENC1(input: List[Char]): List[Either[Char,(Char,Int)]] = {
		...
		accum(input,1)
	}                                         //> runLENC1: (input: List[Char])List[Either[Char,(Char, Int)]]
	val testchar=List('a','b','b','c','c','d','d')
                                                  //> testchar  : List[Char] = List(a, b, b, c, c, d, d)
	runLENC1(testchar)                        //> res0: List[Either[Char,(Char, Int)]] = List(Left(a), Right((b,2)), Right((c
                                                  //| ,2)), Right((d,2)))
